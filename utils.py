...
def downloading_gen2lake(container_name, file_name):
    ...
    try:
        ...
        file = DataLakeFileClient.from_connection_string(account_url, file_system_name=container_name,
                                                         file_path=file_name)
        logger.debug('container_name:{},file_name:{}'.format(container_name, file_name))
        with open(r'C:\Users\Ant PC\PycharmProjects\NewSnowFlakeProject1\{}'.format(file_name), "wb") as my_file:
            download = file.download_file()
            download.readinto(my_file)
            logger.debug('{} downloaded from Gen2Lake'.format(row[1]))
        return my_file
...
def downloading_loading(x):
    ...
    try:
        for row in x.values:
            ...
            logger.debug('{} status {}'.format(row[1],success))
            if row_count == nrows:
                logger.info('All rows loaded into SnowFlake')
            else:
